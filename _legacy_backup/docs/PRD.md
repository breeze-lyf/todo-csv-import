...
1. **自动请求（推荐）**
   - 用户首次登录后，进入日历页面
   - 系统延迟 2 秒后自动弹出浏览器通知权限请求
   - 用户可选择"允许"或"拒绝"
   - 权限状态保存在 localStorage，避免重复请求

2. **手动请求（备选）**
   - 如果用户拒绝或关闭了自动请求
   - 可在"设置"页面手动点击"Enable Notifications"
   - 系统会再次请求权限

3. **权限状态提示**
   - 已授权：显示"✅ Notifications enabled"
   - 已拒绝：显示提示"请在浏览器设置中允许通知"
   - 未请求：自动触发请求流程

**通知发送时机：**

1. 系统后台 Scheduler 每分钟扫描一次 ReminderJob 表
2. 找到 `fireTime <= 当前时间` 且 `sent = false` 的任务
3. 通过 Web Push API 向用户的浏览器发送通知
4. 通知内容：
   - 标题："提醒：事件即将到期"
   - 正文："[事件标题] - [日期]"
   - 点击通知跳转到日历页面
