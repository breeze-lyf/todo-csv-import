...
## **1.6 核心设计原则 (Design Principles)**

- **Glassmorphism (微光特效)**: 使用高透玻璃感、背景模糊和柔和光晕，营造轻盈的现代感。
- **Concise Efficiency (极简高效)**: 提醒显示使用 `[-XD]` 缩写，最大化日历格子内的信息密度。
- **iOS-style Interactivity (直觉交互)**: 借鉴 iOS 的物理动效、圆角逻辑和触控反馈（如 Toggle、Checkbox）。
- **Data Immersion (数据沉浸)**: 通过全站统一的动态弥散背景，让用户专注于日程数据同时拥有极佳的视觉愉悦感。
